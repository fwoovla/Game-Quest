...
public class NpcBeachBum extends Creature {
	...
	public String message[] = {"Sup, Dude!", "I wish I had some rocks to build a house.", "Thanks man!  Here, take this.", "Can you get a few more?"}; 
	
	public int mesgIndex = 0;
	
	private Rectangle radar;

	public NpcBeachBum(Game game, World w, float x, float y) {
	...
	public void hurt(Item i) {
		...
		if (i.name == "rock") {
			if(i.getCount() > 10) {
				mesgIndex = 2;
				world.getEntityManager().getPlayer().getInventory().addItem(new SwordItem(game, world,  "sword", 4));
