...
public class NpcMiner extends Creature {
	...
	public String message[] = {"Hi there", "I'll sell you this pickaxe for 10 bucks.", "Ok, Here you go.", "That's not enough."}; 
	
	public int mesgIndex = 0;
	
	private Rectangle radar;

	public NpcMiner(Game game, World w, float x, float y) {
	...
	public void hurt(Item i) {
		...
		startConversation();
		if (world.getEntityManager().getPlayer().cash >= 10) {
			mesgIndex = 2;
			world.getEntityManager().getPlayer().getInventory().addItem(new PickaxeItem(game, world,  "pickaxe", 5));
			world.getEntityManager().getPlayer().getInventory().removeAllItems(i);
			//}else mesgIndex = 3;
		}
	}
