...
public class NpcDoctor extends Creature {
	...
	public String message[] = {"Doctor at your service. 10 Smackerols", "All fixed up.", "Not enough cash."}; 
	
	public int mesgIndex = 0;
	
	private Rectangle radar;

	public NpcDoctor(Game game, World w, float x, float y) {
	...
	public void hurt(Item i) {
		...
		if (world.getEntityManager().getPlayer().cash >= 10) {
			mesgIndex = 1;
			world.getEntityManager().getPlayer().health = 100;
			world.getEntityManager().getPlayer().cash-=10;
		}else mesgIndex = 2;
	}
